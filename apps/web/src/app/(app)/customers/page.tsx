...
const emptyForm: CustomerForm = { name: '', taxNumber: '', email: '', phone: '' };
...
export default function CustomersPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api.get<PaginatedResponse<Customer>>(
        `/customers?limit=50${search ? `&search=${encodeURIComponent(search)}` : ''}`,
      ),
  });
  ...
  const columns: Column<Customer>[] = [
    ...
    { key: 'phone', header: t('common.phone'), render: (row) => row.phone ?? '-' },
    {
      ...
      render: (row) => formatCurrency(row.creditLimit, row.currency?.code ?? 'TRY'),
    },
  ];
